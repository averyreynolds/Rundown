...
Two categories, each mapped to exactly one HTTP status by
`app/api/error_handlers.py`, so every router surfaces provider failures
the same typed way instead of each repeating its own translation logic
(CLAUDE.md hard rule 6 / the plan's System-Wide Impact invariant: external
failures surface as typed responses, never raw exceptions or stack
traces):
