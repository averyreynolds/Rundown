...
Two tables: a generic `cache_entries` table (CLAUDE.md's "one caching
module" instruction, and the Key Technical Decision to avoid bespoke
tables per provider) and `portfolio_snapshots` (R12's minimal daily
snapshot-write job, populated by U9's scheduled job, queried by nothing
yet -- comparison logic is deferred follow-up work).

There is deliberately no SnapTrade connection/credential table: Personal-
tier SnapTrade auth has no local user-registration step to persist a
secret for -- see `app/services/snaptrade_service.py`'s module docstring.
"""
