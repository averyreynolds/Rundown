...
- **The advisor sees only part of a filing, and some of what it sees is a pointer rather than content.** `app/domain/filing_sections.py` extracts the portfolio-relevant Items (10-K: 1A, 3, 5, 7, 7A; 10-Q: Part I Items 2–3, Part II Items 1 and 1A; 8-K whole) and drops Item 1 Business and Item 8 Financial Statements. Separately, SEC rules let a filer satisfy an Item by cross-referencing a note, exhibit, or prior filing instead of restating it — in JPMorgan's 10-K, Item 7 (MD&A) is a 300-character page reference and Item 3 is 76 characters. Those sections are detected by length and labeled as pointers in the model's context, so the advisor won't present a page number as management's discussion, but *the material they point to is not fetched*. Following cross-references into financial statement notes and exhibits is deferred follow-up work. Verified against 14 real filings from 7 filers (Apple, JPMorgan, P&G, Tesla, Coca-Cola, UnitedHealth); segmentation found every targeted section in all of them.
