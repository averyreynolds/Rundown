...
*implemented* router (portfolio, fundamentals, filings, news), with every
external call mocked. U8 (the AI advisor) was deferred by user request --
see `backend/README.md`'s "Current status" section -- so there is no
`/advisor` route to include here yet; add one once that unit ships.
"""
...
_IMPLEMENTED_ROUTE_PREFIXES = ("/health", "/portfolio", "/fundamentals", "/filings", "/news")
...
def test_one_route_per_router_succeeds_with_bearer_token(
    ...
    routes_and_expected_schema_keys = {
    ...
    for path, expected_keys in routes_and_expected_schema_keys.items():
