...
def _app_state_with_holdings(
    db_session_factory: async_sessionmaker[AsyncSession],
    *,
    positions: list[dict[str, Any]] | None = None,
) -> SimpleNamespace:
    """Build a fake `app_state` with the given holdings pre-loaded in the SnapTrade fake.

    Personal-key flow: credentials come from `get_settings()` at job
    run-time (set to placeholders by conftest.py), so no `connect()` call
    or DB row is needed here -- the scheduler constructs `SnapTradeService`
    directly with those env-sourced credentials.
    """
...
async def test_refresh_fundamentals_writes_one_cache_entry_per_symbol(
    ...
    app_state = _app_state_with_holdings(db_session_factory)
...
async def test_refresh_fundamentals_one_symbol_failure_does_not_abort_batch(
    ...
    app_state = _app_state_with_holdings(
        db_session_factory,
        positions=[
...
async def test_write_daily_snapshot_writes_nothing_when_no_brokerage_linked(
    db_session_factory: async_sessionmaker[AsyncSession],
) -> None:
    """No brokerage accounts linked yet: SnapTrade returns [] → no snapshot rows written."""
    app_state = SimpleNamespace(
        session_factory=db_session_factory,
        snaptrade_client=build_fake_snaptrade_client(),  # empty accounts by default
    )
